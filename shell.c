...
node* rm(node* n, char* path) {
/* Removes the file or directory at the end of the specified path, and returns
 * pointer to it's parent directory. Directories can't be removed unless empty*/

	node* temp = n;
	n = findNode(temp, path);
	// check if node is root
	if (n->parent == NULL)
		return NULL;
	// check if path exists
	if (n == NULL) {		
		printf("Path note found\n");
		return temp;
	}
	// check if nonempty directory
	if (n->nodeType == 1 && n->firstChild != NULL) {
		printf("Error: Directory must be empty\n");
		return temp;
	}
	// removal for node as firstChild
	if (strcmp(n->parent->firstChild->name, n->name) == 0) {
		temp = n;
		n->parent->firstChild = n->sibling;
		free(temp);
	}
	// removal for node as a member
	else {
		temp = n;
		n = n->sibling;
		free(temp);
